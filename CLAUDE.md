...
| Package                  | Purpose                                                                               |
| ------------------------ | ------------------------------------------------------------------------------------- |
| `packages/component-doc` | Types-only schema (`ComponentDoc`) for colocated component spec docs                  |
...
Build order is enforced: `component-doc → tokens → core → agents → mcp-builder`. Always run `npm run build` from the root.
...
Every implemented component needs a spec doc colocated with its source: `packages/<package>/src/[ComponentName]/[ComponentName].doc.ts`, a typed `ComponentDoc` object (schema in `packages/component-doc`). See `packages/core/src/Button/Button.doc.ts` for the format. All current components have spec docs.

Spec docs are the single source of truth for the MCP server's component metadata: `packages/mcp-builder/src/metadata/components.ts` is generated by importing every `packages/{core,agents}/src/**/*.doc.ts` directly (`npm run metadata:generate -w packages/mcp-builder`, also runs during that package's build) — never hand-edit it; CI fails if it is stale. There is no markdown to parse — `tsc` and `eslint` are the gates (a `.doc.ts` file must type-check as `ComponentDoc`, and its prose fields are subject to the same token-usage rules as component source). See the `add-component-spec` skill for the shape.
...
- `/add-component-spec <ComponentName>` — colocated spec doc at `<Name>.doc.ts` for an existing component
...
- `eslint-plugin-import-x` — no cycles, no cross-package relative imports, no undeclared dependencies (respects agents' peerDependencies; core's tokens devDep is whitelisted because tsup inlines it; both core's and agents' `component-doc` devDep is whitelisted because `*.doc.ts` files only ever `import type` from it)
