...
| Fix WCAG AA color-contrast failures blocking the last 6 light-mode baselines (added 2026-08-18): `color.text.muted` (`CodeBlock` language label, `packages/core/src/CodeBlock/CodeBlock.tsx`) against `neutralWarmth`-shifted light surfaces (`AgenticProvider` `WarmNeutrals`/`CoolNeutrals` stories), and `color.agent.status.waiting` against the light-mode badge background (`AgentStatus` `Waiting` story). Axe-core caught both via the `postVisit` a11y gate before any screenshot was taken — re-run `npm run test:visual:light:update:docker` once fixed | High     |
| Wire `npm run test:visual:light` into `ci.yml` as a PR gate (added 2026-08-18) — light-mode baselines exist now (90/96, `apps/storybook/.storybook/test-runner.ts` `STORYBOOK_COLOR_SCHEME=light`) but nothing currently re-checks them on every PR the way the dark-mode `test:visual` step does                                                                                                                                                                                                                                                                  | Medium   |
