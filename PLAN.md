...
| Wire `npm run test:visual:light` into `ci.yml` as a PR gate (added 2026-08-18) — light-mode baselines exist now (96/96 as of 2026-08-20, `apps/storybook/.storybook/test-runner.ts` `STORYBOOK_COLOR_SCHEME=light`) but nothing currently re-checks them on every PR the way the dark-mode `test:visual` step does | Medium   |
